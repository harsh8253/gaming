...
export function SettlementsPage() {
  ...
  return (
    <>
      <PageHeader title="Settlements" description="Review and approve settlement requests raised by masters." />
      ...
      <section className="mt-6 rounded-lg border border-border bg-card">
        ...
        {filtered.length === 0 ? (
          ...
          <>
          <MobileList label="Settlements">
            {filtered.map((settlement) => (
              <ListRow
                key={settlement.id}
                onClick={() => setSelectedId(settlement.id)}
                title={<FixtureLabel match={settlement.match} />}
                subtitle={`${settlement.id} · ${settlement.betsCount} bets · ${settlement.requestedBy}`}
                meta={<span>{settlement.requestedAt}</span>}
                trailing={formatINR(settlement.totalPayout)}
                trailingSub={
                ...
                {filtered.map((settlement) => (
                  ...
                  >
                    <td className="px-5 py-3.5">
                      <p className="font-mono text-[11px] font-semibold text-blue-600">{settlement.id}</p>
                    ...
                    <td className="px-5 py-3.5 text-right text-[11px] text-muted-foreground">{settlement.requestedAt}</td>
                  </tr>
                ))}
              </tbody>
            </StackTable>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5">
            <dl className="grid grid-cols-2 gap-3">
              <div className="rounded-lg bg-slate-50 px-3 py-3">
                <dt className="text-[10px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">Bets included</dt>
                <dd className="mt-1 text-[13px] font-semibold tabular-nums text-slate-900">{selected.betsCount}</dd>
              </div>
              <div className="rounded-lg bg-slate-50 px-3 py-3">
                <dt className="text-[10px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">Total payout</dt>
            ...
            <div className="mt-5 space-y-2 text-[12px] text-slate-600">
              <div className="flex items-center gap-2">
                <User size={13} className="text-muted-foreground" /> Requested by {selected.requestedBy}
              </div>
              <div className="flex items-center gap-2">
                <CalendarClock size={13} className="text-muted-foreground" /> {selected.requestedAt}
              </div>
            </div>
          </div>
